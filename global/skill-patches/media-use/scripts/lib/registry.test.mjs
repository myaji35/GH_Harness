...
test("sanctioned providers only: heygen, local mflux/kokoro/edge/vibevoice/ltx, codex, design spec, logo tiers", () => {
  const allowed =
    /^heygen|^bundled\.sfx$|^mflux\.local$|^kokoro\.local$|^edge\.tts$|^vibevoice\.local$|^ltx\.local$|^codex\.image_gen$|^design_spec$|^svgl$|^simple-icons$|^github\.avatar$|^favicon\.ddg$|^color_grade\.local$|^cube_lut\.local$/;
