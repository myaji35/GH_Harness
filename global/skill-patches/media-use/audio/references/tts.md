...
| 4     | MS TTS (Edge)     | `edge-tts` CLI installed (no key required)  | `ko-KR-SunHiNeural`, `ko-KR-InJoonNeural`, `ko-KR-HyunsuMultilingualNeural`, plus many multilingual voices (`edge-tts --list-voices`) | No | mp3 (`ffprobe`-measured duration) |

### MS TTS (Edge)

Install with `pip install edge-tts`. **No API key is required**, which conforms
to media-use's CLI-only invariant: the skill does not hold keys.

**한국어 품질이 좋아 국내 콘텐츠 나레이션의 기본 선택지다.** Three Korean
voices are available: female SunHi (`ko-KR-SunHiNeural`), male InJoon
(`ko-KR-InJoonNeural`), and male multilingual Hyunsu
(`ko-KR-HyunsuMultilingualNeural`). When the intent contains Hangul, the adapter
automatically selects `ko-KR-SunHiNeural`; for primarily Latin text, it selects
`en-US-AriaNeural`. Set `ctx.voiceId` to choose a voice explicitly.

Use `ctx.rate`, `ctx.pitch`, and `ctx.volume` to adjust speed, pitch, and volume.
Values are signed strings such as `+20%` or `-5Hz`. When calling `edge-tts`
directly from a shell, negative values must use the equals form, for example
`--rate="-10%"`; `--rate "-10%"` causes an argument-parsing error.

```bash
edge-tts --text "안녕하십니까" --voice ko-KR-SunHiNeural --write-media narration.mp3
```

If `edge-tts` is not installed, the adapter returns `null` and quietly falls
back to the next provider.

`vibevoice.local` is a local backend for voice cloning from a 30-second sample
and up to four speakers; only its skeleton is implemented. It attempts to run
only when `VIBEVOICE_HOME` is set, otherwise returning `null` to fall back.
**Do not use it for customer-facing commercial services until its license has
been verified.** Microsoft removed the official TTS code over deepfake concerns,
and the working implementation is a community copy. Restrict it to internal
production and label generated audio as AI-generated.
