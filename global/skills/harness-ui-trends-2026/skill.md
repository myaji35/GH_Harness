...
5. **`industry` 필드가 있으면 섹션 9의 업종 프리셋을 `anti_patterns`/권장값에 자동 시드**한다 (아래 참조).

---

## 9. 업종별 안티패턴 프리셋 (Industry Presets)

> **출처**: Eric Park "UI/UX Pro Max" 분석 영상(2026-07). 핵심 원리 — **업종이 정해지면 UI 스타일·컬러·모션이 연쇄로 잠기고, 특히 "금지 목록"이 AI 기본 출력의 열화(보라-핑크 그라데이션 + 이모지 남발)를 사전 차단**한다.
> **철학**: 룰은 정답이 아니라 **시작점**이다. 아래 프리셋은 하한선일 뿐, 실제 앱 레퍼런스(Mobbin 등)로 **이중 검증** 후 프로젝트 톤에 맞게 조정한다. 영미권 폰트 룰은 한글에 1:1 적용 금지(조합 원리만 참고).

### 9-1. 연동 방식
- `brand-dna.json`에 `"industry": "<key>"` 필드가 있으면, brand-guardian(또는 세션 시작 시)이 해당 프리셋의 `must_avoid`를 프로젝트 `anti_patterns` 배열에 **머지**(중복 제외)한다.
- `recommend`는 design_tokens 미설정 항목의 기본값 힌트로만 쓴다(기존 프로젝트 값이 우선).
- design-critic / brand-guard.sh는 리뷰 시 `must_avoid` 항목 위반을 **자동 감점**한다.
- `industry` 미설정 시 프리셋 미적용(기존 동작 유지). **오버 방지 원칙**: 프리셋은 옵트인이다.

### 9-2. 프리셋 라이브러리

| industry key | 핵심 가치 | recommend (권장) | **must_avoid (금지)** |
|---|---|---|---|
| `finance_dashboard` | 신뢰·속도 | 다크모드 기본, 빠른 렌더/애니메이션, 밀도 높은 데이터 | 느린 애니메이션(금전손실 직결), AI 보라-핑크 그라데이션, 이모지 아이콘 |
| `banking` | 신뢰·권위 | 미니멀, 네이비+골드, 보안/인증 표시 | 장난스러운 UI(클레이모피즘·사이버펑크), 허술한 UX, AI 보라-핑크 그라데이션 |
| `insurance` | 신뢰·명료 | 미니멀·구조적, 관계 시각화(그래프), 차분한 블루 계열 | 과한 모션, 장난스러움, 이모지, AI 보라-핑크 그라데이션 |
| `certification` | 성취·단계 | 진행/성취 시각화(Path·Stepper), 명확한 다음 단계 CTA | 정보 과밀로 단계 흐림, 저대비 텍스트, 이모지 |
| `community` | 친근·접근성 | 모바일 반응형 우선, 따뜻한 톤, 큰 터치 타깃 | 저대비/작은 폰트(접근성 위반), 데스크톱 전용 레이아웃 |
| `saas` | 명료·효율 | Calm Design, 화면당 CTA 1개, ⌘K, 중립색+주색1 | AI 배지 과장, 정보 과밀, 보라-핑크 그라데이션 |
| `kids_edu` | 활력·재미 | 밝은 원색, 통통 튀는 인터랙션(클레이모피즘), SVG 마스코트 | 차분·저채도·저에너지, 이모지로 캐릭터 대체, 삐뚤한 정렬 |
| `meditation` | 차분·집중 | 파스텔·톤다운, 아이콘(이모지 아님), 절제된 여백 | 과한 모션·네온, 보라-핑크 그라데이션, 정신 사나운 밀도. ※뉴모피즘은 실사례 드묾 — 맹신 금지 |
| `luxury` | 우아·여유 | 느긋한 모션("비쌀수록 느리게"), 세리프 헤딩+가독 본문, 넉넉한 여백 | 빠른/촐싹대는 애니메이션, 시장바닥식 밀도, 원색 남발 |
| `game_dev_tool` | 몰입·해커감성 | 다크 배경, 네온/사이버펑크 허용, 모노스페이스 | 과한 파스텔, 엔터프라이즈식 보수적 톤 |

### 9-3. 전 업종 공통 금지 (Universal Anti-Patterns)
아래는 `industry` 무관 **항상 적용**되는 금지 항목(섹션 3 가독성 규칙과 병행):
- ❌ **AI 보라-핑크 그라데이션** — AI 기본 출력의 대표적 열화 신호. 전 업종 금지.
- ❌ **UI 아이콘 자리에 이모지** — 섹션 3-4 line icon 규칙과 동일. 아이콘으로 대체.
- ❌ **정렬 삐뚤·이 빠진 그리드** — 홀수 박스는 2×2 등 안정 배치로.
- ❌ **프라이머리 버튼 색상 불일치** — 가장 중요한 CTA는 전 화면 일관.

### 9-4. brand-dna.json 스키마 확장(예시)
```json
{
  "industry": "insurance",
  "anti_patterns": ["과한 모션", "AI 보라-핑크 그라데이션"],
  "_industry_preset_seeded": true
}
```
- `industry` 값이 섹션 9-2의 key와 일치하면 프리셋 자동 시드.
- 포트폴리오 매핑: InsureGraph Pro=`insurance`, CertiGraph=`certification`, Townin=`community`.
